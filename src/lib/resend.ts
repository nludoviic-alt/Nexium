...
/**
 * 9. CODE OTP DE VÉRIFICATION D'E-MAIL LORS DE L'INSCRIPTION
 */
export function renderEmailVerificationOtpHtml(
  clientName: string,
  otpCode: string,
  lang: "fr" | "en" = "fr"
): string {
  ...
  const content = `
    ${renderIntroSection(
      "🔐",
      isFr ? "Vérification de votre adresse e-mail" : "Verify Your Email Address",
      isFr
        ? `Bonjour <strong style="color:#17212B;font-size:18px;">${clientName}</strong>,<br>Pour finaliser l'ouverture de votre compte de trading et confirmer votre identité, veuillez saisir le code de sécurité temporaire ci-dessous :`
        : `Hello <strong style="color:#17212B;font-size:18px;">${clientName}</strong>,<br>To finalize your trading account creation and verify your identity, please enter the temporary security code below:`
    )}
          ...
          <tr>
            <td align="center" style="padding:32px 24px;">
              <div style="
                ...
                text-transform:uppercase;
              ">
                ${isFr ? "Code de vérification temporaire" : "Temporary Verification Code"}
              </div>

              <div style="
                display:inline-block;
                background:#FFFFFF;
                border:2px dashed #00C98D;
                border-radius:14px;
                padding:16px 36px;
                box-shadow:0 4px 14px rgba(0,201,141,0.12);
              ">
                <span style="
              ...
              <div style="
                ...
                color:#718096;
              ">
                ⏱️ ${isFr ? "Valable pendant 10 minutes" : "Valid for 10 minutes"}
              </div>
            </td>
          </tr>
        </table>
    ...
    ${renderCalloutSection(
      isFr ? "Sécurité & Confidentialité" : "Security & Privacy",
      isFr
        ? "Ne partagez jamais ce code. Les conseillers Nexium Markets ne vous demanderont jamais votre code de vérification par téléphone ou message."
        : "Never share this code. Nexium Markets advisors will never ask for your verification code by phone or direct message.",
      "#F0FDF4",
      "#00C98D",
      "#065F46"
    )}
  ...
  return getEmailWrapper(
    isFr ? "Nexium Markets — Code de vérification e-mail" : "Nexium Markets — Email Verification Code",
    isFr
      ? `${otpCode} est votre code de vérification pour finaliser votre inscription Nexium Markets.`
      : `${otpCode} is your verification code to finalize your Nexium Markets registration.`,
    content,
    { kicker: isFr ? "Sécurité Compte" : "Account Security", lang }
  );
}
...
export async function sendEmailVerificationOtp(params: {
  ...
  const subject =
    lang === "fr"
      ? `🔐 ${params.code} est votre code de vérification Nexium Markets`
      : `🔐 ${params.code} is your Nexium Markets verification code`;
  const html = renderEmailVerificationOtpHtml(params.fullName, params.code, lang);
  return sendViaResendHttp(params.email, subject, html);
}
